...
class Settings(BaseSettings):
    ...
    bug_hunter_languages: list[str] = ["python", "javascript", "typescript", "jsx", "tsx", "c", "cpp"]
    bug_hunter_delay_seconds: float = 1.0    # pause between files to respect rate limits
    bug_hunter_related_files: int = 2        # focused cross-file confirmation context
    bug_hunter_related_chars: int = 8_000

    # Python change-impact analysis
    impact_max_hops: int = 3
    impact_max_related_files: int = 6
    ...
    # Paths
    # Anchored to the application, so launching from another working directory
    # cannot accidentally create a second memory database.
    chroma_db_path: str = str(APP_ROOT / "chroma_db")
    ...
    validation_run_full_suite: bool = True
