...
class ConfidenceScorer:
    ...
    def compute(
        ...
        score.tests_available = validation.tests_available and validation.test_runner_available
        score.security_scanner_available = validation.security_scanner_available

        # Gate 2: tests passed
        score.tests_signal = (
            self.WEIGHTS["tests"]
            if validation.test_runner_available and validation.gate_2_tests_passed
            else 0.0
        )

        # Gate 4: security clean
        score.security_clean_signal = (
            self.WEIGHTS["security_clean"]
            if validation.security_scanner_available and validation.gate_4_security_clean
            else 0.0
        )
