...
class _StripAnthropicUATransport(httpx.AsyncHTTPTransport):
    """Transport that replaces the 'Anthropic/Python ...' user-agent before the
    request leaves the process.

    The Anthropic SDK always prepends its own identifier to whatever user-agent
    the caller sets (via ``default_headers``), producing strings like
    ``Anthropic/Python 0.111.0, python-httpx/0.28.1``. Some proxies (e.g.
    Aerolink) block any request whose User-Agent contains ``Anthropic/Python``.
    This transport intercepts the request at the lowest httpx layer and replaces
    the header with a plain httpx identifier before it hits the wire.
    """
...
class LLMClient:
    ...
    def __init__(self):
        ...
        # Route through a custom Anthropic-compatible endpoint (e.g. Aerolink, Lightning AI)
        # when ANTHROPIC_BASE_URL is configured.
        if settings.anthropic_base_url:
            kwargs["base_url"] = settings.anthropic_base_url
            # Aerolink blocks requests whose User-Agent contains "Anthropic/Python".
            # The SDK always prepends its own UA string, so default_headers alone won't
            # fix it. We need a transport-level override that replaces the header entirely
            # before the request leaves the process.
            kwargs["http_client"] = httpx.AsyncClient(
                transport=_StripAnthropicUATransport()
            )
    ...
    async def call(
        ...
        for attempt in range(settings.llm_max_retries):
            try:
                ...
                anthropic.BadRequestError,       # 400 — malformed request, won't improve
                anthropic.AuthenticationError,   # 401 — bad API key
                anthropic.PermissionDeniedError, # 403
                anthropic.NotFoundError,         # 404 — bad model name
            ) as e:
                # Non-retryable: retrying burns time and money for the same error.
                raise LLMError(f"LLM call failed (non-retryable): {e}")

            except Exception as e:
                # Retryable: rate limits, overloaded, timeouts, transient network.
