...
class OrchestratorAgent:
    ...
    def __init__(self):
        ...
        self.bug_investigation = BugInvestigationAgent(self.memory)
    ...
    async def run_pipeline(
        ...
        try:
            ...
            # The bug hunter needs the knowledge graph, so it chains after the
            # repo mapper; that chain, the dependency analyzer, and static
            # analysis are independent — run all three concurrently.
            async def _map_then_hunt():
                ...
                return kg, findings_kg, findings_llm

            (kg, findings_kg, findings_llm), findings_dep, findings_static = await asyncio.gather(
                _map_then_hunt(),
                self.dep_analyzer.run(str(repo_path), event_callback),
                self.static_analysis.run(str(repo_path), event_callback),
            )

            all_findings = findings_kg + findings_dep + findings_static + findings_llm
            self._normalize_finding_paths(all_findings, repo_path)
            ...
            job.findings = await self.bug_investigation.run(
                fixable, kg, event_callback, repo_key=f"{repo_owner}/{repo_name}",
            )
            ...
            # Windows-compatible cleanup: force garbage collection, wait, then retry deletion
            gc.collect()
            time.sleep(0.5)
            try:
                shutil.rmtree(tmpdir, ignore_errors=False)
            except Exception:
                # If deletion still fails, try one more time after a longer wait
                time.sleep(1.0)
                try:
                    shutil.rmtree(tmpdir, ignore_errors=True)
                except Exception:
                    pass  # Best effort - temp files will be cleaned up by OS eventually
    ...
    async def _fix_validate_loop(
        ...
        for attempt in range(settings.validation_max_retries):
            ...
            validation.security_scanner_available = security.security_scanner_available
    ...
    def _related_files(self, file_path: str, kg, limit: int = 3) -> list[str]:
        """Graph-selected Python dependencies, consumers, and relevant tests."""
        try:
            neighbours = kg.related_files(
                file_path,
                hops=settings.impact_max_hops,
                limit=settings.impact_max_related_files,
            )
        except Exception:
            return []
        out: list[str] = []
        for n in neighbours:
            ...
            if attrs.get("type") == "File" and str(n).endswith(".py"):
                out.append(n)
        return out[:settings.impact_max_related_files]

    @staticmethod
    def _normalize_finding_paths(findings: list[Finding], repo_path: Path) -> None:
        """Convert scanner paths to the graph's relative POSIX format."""
        ...
        for finding in findings:
            ...
            try:
                ...
                # so it would turn ".github/x.yml" into "github/x.yml".
                finding.file_path = finding.file_path.replace("\\", "/").removeprefix("./")
    ...
    def _dedupe(self, findings: list[Finding]) -> list[Finding]:
        ...
        Two passes: exact (file, line, class, title), then (file, line, class)
        for findings with a concrete line — different tools describe the same
        defect with different titles (e.g. the repo mapper's deterministic
        syntax-error finding vs the LLM hunter's). First occurrence wins, and
        deterministic tools run before the LLM hunter merges its results.
        """
        seen: set[tuple] = set()
        ...
        for f in findings:
            ...
            loc_key = (f.file_path, f.line_number, f.issue_class.value)
            if key in seen:
                continue
            if f.line_number is not None and loc_key in seen_loc:
                continue
            seen.add(key)
    ...
    def _remember(
        ...
        try:
            self.memory.store_fix(
                ...
                files_changed=patch.changed_files,
                validation_summary=(
                    f"ast={validation.gate_1_ast_valid};"
                    f"tests={validation.gate_2_tests_passed};"
                    f"security={validation.gate_4_security_clean};"
                    f"security_scanner={validation.security_scanner_available}"
                ),
            )
        except Exception:
