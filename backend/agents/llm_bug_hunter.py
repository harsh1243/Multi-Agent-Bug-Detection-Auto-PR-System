...
class LLMBugHunterAgent:
    ...
    async def run(
        ...
        for idx, rel in enumerate(files, start=1):
            ...
            if not code.strip():
                continue
            graph_context = kg.context_summary(
                rel, hops=settings.impact_max_hops,
            )

            # ── Stage A: cheap triage ──
            try:
                candidates = await self._triage(rel, code, graph_context)
            ...
            try:
                related_context = self._related_source_context(repo_path, rel, kg)
                confirmed = await self._confirm(
                    rel, code, candidates, graph_context, related_context,
                )
            except (LLMError, ValueError) as e:
                await self._emit(
                    event_emitter, "agent_warning",
    ...
    async def _triage(self, rel_path: str, code: str, graph_context: str) -> list[dict]:
...
Relevant dependent/dependency files (use these to verify contracts; do not invent
relationships beyond what is shown):
{related_context or 'No additional file content available.'}
...
If none are real, return []."""
    ...
    def _related_source_context(
        self, repo_path: str, rel_path: str, kg: KnowledgeGraph,
    ) -> str:
        """Read a very small set of graph-selected files for confirmation only."""
        root = Path(repo_path).resolve()
        chunks: list[str] = []
        related = kg.related_files(
            rel_path,
            hops=settings.impact_max_hops,
            limit=settings.bug_hunter_related_files,
        )
        for related_path in related:
            target = (root / related_path).resolve()
            try:
                target.relative_to(root)
                text = target.read_text(encoding="utf-8", errors="ignore")
            except (OSError, ValueError):
                continue
            text = text[:settings.bug_hunter_related_chars]
            chunks.append(f"### {related_path}\n```\n{text}\n```")
        return "\n\n".join(chunks)
