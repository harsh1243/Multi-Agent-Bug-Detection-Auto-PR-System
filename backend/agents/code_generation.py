...
- If a related test file is shown, add or update a focused regression test when
  practical. Never delete or weaken an existing assertion to make validation pass.
...
- Every emitted edit is atomic: if even one block cannot be applied, the whole
  patch is rejected and retried. Change only what is required. Do not touch
  unrelated lines, imports, whitespace, or formatting. Output nothing except
  `### FILE:` lines and the blocks."""
...
class CodeGenerationAgent:
    ...
    def _collect_files(self, primary: str, related: list[str], repo_path: str) -> dict[str, str]:
        ...
        try:
            primary_path = self._safe_repo_path(root, primary)
            contents[primary] = primary_path.read_text(encoding="utf-8", errors="ignore")
        ...
        for rel in related:
            if rel == primary or rel in contents:
                continue
            try:
                p = self._safe_repo_path(root, rel)
            except ValueError:
                continue
            try:
    ...
    def _safe_repo_path(root: Path, relative: str) -> Path:
        """Resolve a path and reject absolute/path-traversal targets."""
        raw = Path(relative)
        if raw.is_absolute():
    ...
    def _build_result(self, primary_path: str, file_contents: dict[str, str], raw: str) -> PatchResult:
        ...
        if not grouped:
            errors.append("No SEARCH/REPLACE edits found in the model output.")
        ...
        for path, edits in grouped.items():
            ...
            if ar.applied > 0 and ar.new_content != original:
        ...
        complete = edits_total > 0 and applied_total == edits_total and not errors
        return PatchResult(
            file_path=primary_path,
            ok=complete and len(file_patches) > 0,
            files=file_patches,
    ...
    def _norm(p: str) -> str:
        ...
        # would turn ".env" into "env" and never match the real file.
        return p.replace("\\", "/").strip().removeprefix("./").lower()
