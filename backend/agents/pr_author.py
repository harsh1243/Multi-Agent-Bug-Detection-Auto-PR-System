...
from models import (
    ConfidenceScore, Finding, PatchResult, PullRequest, PipelineEvent,
    ValidationResult,
)
...
class PRAuthorAgent:
    ...
    async def create_pr(
        ...
        for f in findings:
            modified_funcs.extend(f.affected_symbols)
        ...
        description = await self._generate_description(
            file_path, findings, confidence, patch, validation,
        )
        ...
        branch_name = f"fix/{primary.id}-{datetime.now(timezone.utc):%Y%m%d-%H%M%S}"
        pr_url = ""
        wrote = False
        try:
            ...
            # Always leave the worktree clean and back on the base branch so a
            # failed attempt's writes can never leak into the next PR's commit.
            try:
                self.github.restore_worktree(Path(repo_path), base_branch)
            except Exception:
                pass

        # This field describes the actual diff only. Potentially affected
        # consumers belong in the blast-radius section, not the changed-file list.
    ...
    def _apply_patch_to_worktree(self, patch: PatchResult, repo_path: str) -> bool:
        ...
        for fp in patch.files:
            target = (root / fp.file_path).resolve()
            try:
...
Describe what was wrong and how the change fixes it at the source. Be specific and technical."""
        ...
        test_status = "✅" if confidence.tests_signal > 0 else (
            "N/A (no tests)" if not validation.tests_available else "Unavailable/failed"
        )
        security_status = "✅" if confidence.security_clean_signal > 0 else (
            "Unavailable" if not validation.security_scanner_available else "Failed"
        )
        signals = (
            f"| Tests Passed | +40% | {test_status} |\n"
            f"| Security Clean | +25% | {security_status} |\n"
